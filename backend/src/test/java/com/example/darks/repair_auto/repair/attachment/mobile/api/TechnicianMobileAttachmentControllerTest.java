...
class TechnicianMobileAttachmentControllerTest {
    ...
    void setUp() {
        ...
        TechnicianMobileAttachmentController controller = new TechnicianMobileAttachmentController(facade);
    ...
    @Test
    void givenTechnicianAuth_whenPostAttachment_thenReturns201Created() throws Exception {
        MockMultipartFile file = new MockMultipartFile(
                "file", "diagnosis.jpg", "image/jpeg", new byte[]{1, 2, 3});
        ...
        mockMvc.perform(multipart("/api/v1/mobile/me/jobs/42/attachments")
                        .file(file)
                        .param("attachmentType", "DIAGNOSIS_PHOTO"))
    ...
    @Test
    void givenTechnicianAuth_whenGetAttachments_thenReturns200List() throws Exception {
        MobileAttachmentResponse response = new MobileAttachmentResponse(
                502L,
                42L,
                AttachmentType.DIAGNOSIS_PHOTO,
        ...
        when(facade.listTechnicianAttachments(eq(currentActor), eq(42L)))
                .thenReturn(List.of(response));

        mockMvc.perform(get("/api/v1/mobile/me/jobs/42/attachments"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].id").value(502))
                .andExpect(jsonPath("$[0].type").value("DIAGNOSIS_PHOTO"));
    }

    @Test
    void givenCrossTechnicianOrNonExistent_whenGetAttachments_thenReturns404() throws Exception {
        when(facade.listTechnicianAttachments(eq(currentActor), eq(999L)))
                .thenThrow(new BusinessException(ErrorCode.REPAIR_REQUEST_NOT_FOUND));

        mockMvc.perform(get("/api/v1/mobile/me/jobs/999/attachments"))
